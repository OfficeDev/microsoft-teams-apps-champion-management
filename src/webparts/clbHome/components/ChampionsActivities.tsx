...
export default class ChampionsActivities extends Component<
  ChampionsActivitiesProps,
  ChampionsActivitiesState
> {
  ...
  public componentDidUpdate(
    prevProps: Readonly<ChampionsActivitiesProps>,
    prevState: Readonly<ChampionsActivitiesState>,
    snapshot?: any
  ): void {
  ...
  public async getChampionPendingEvents() {
    try {
      ...
      let pendingEventsFilterQuery =
        "Status eq '" + stringConstants.pendingStatus + "'";
      const sortColumn = "Created";
      const pendingEvents: any[] =
        await this.commonServiceManager.getItemsSortedWithFilter(
          stringConstants.EventTrackDetailsList,
          pendingEventsFilterQuery,
          sortColumn
        );
      ...
      const filteredMemberIds = memberIds.filter(
        (item: any, index: number) => memberIds.indexOf(item) === index
      );
      ...
      filteredMemberIds.forEach((id: any, idx: number) => {
        memberIdFilterQuery =
          idx === 0 ? "ID eq " + id : memberIdFilterQuery + " or ID eq " + id;
      });
      //Getting data from member list
      const memberData: any[] =
        await this.commonServiceManager.getItemsWithOnlyFilter(
          stringConstants.MemberList,
          memberIdFilterQuery
        );
      ...
      this.setState({
        championPendingEvents: championEvents,
        showSpinner: false,
      });
    } catch (error) {
      console.error(
        "CMP_ChampionActivities_getChampionPendingEvents \n",
  ...
  private async updatePendingEvents(statusText: string, selectedIDs: any) {
    try {
      ...
      let updateResponse = await this.commonServiceManager.updateMultipleItems(
        stringConstants.EventTrackDetailsList,
        updateEventObject,
        selectedIDs
      );
      ...
      if (updateResponse) {
        ...
        this.setState({
          rejectMessage:
            ...
            "while approving/rejecting champion event request(s).",
          selectedIds: [],
          isAllSelected: false,
          showSpinner: false,
        });
      ...
      this.setState({
        rejectMessage:
          stringConstants.CMPErrorMessage +
          "while approving/rejecting champion event request(s). Below are the details: \n" +
          JSON.stringify(error),
        selectedIds: [],
        isAllSelected: false,
        showSpinner: false,
      ...
      console.error(
        "CMP_ChampionActivities_updatePendingEvents \n",
        JSON.stringify(error)
      );
    }
  }
  ...
  public selectChampionEvents(
    ...
    if (selectAll && isChecked) {
      ...
      this.state.filteredChampionPendingEvents.forEach(
        (event: IChampionPendingEvent) => {
    ...
    // When "Select All" is unchecked
      ...
      if (isChecked) {
        ...
        if (
          ...
          this.state.filteredChampionPendingEvents.length
        ) {
          this.setState({ isAllSelected: true });
        }
      }
          ...
          return (
            <button
              ...
              className={`btn${
                ...
                this.props.currentThemeName === stringConstants.themeDefaultMode
                  ? ""
                  : " selected-darkcontrast"
              }`}
              aria-label={
                isSelect
                  ? stringConstants.sizePerPageLabel +
                    option.text +
                    stringConstants.selectedAriaLabel
                  : stringConstants.sizePerPageLabel + option.text
              }
            >
              {option.text}
            </button>
      ...
      return (
        ...
        >
          <a
            ...
            aria-label={
              options.active
                ? ariaLabel + stringConstants.selectedAriaLabel
                : ariaLabel
            }
          >
    ...
    const eventsTableHeader: any = [
      {
        ...
        formatter: (_: any, gridRow: any) => {
          return (
            <Checkbox
              onChange={(_eve: any, isChecked: boolean) => {
                this.selectChampionEvents(
                  isChecked,
                  gridRow.EventActivityId,
                  false
                );
              }}
      ...
      },
      {
        ...
        sort: true,
        sortCaret: this.customSortCaret,
        formatter: (_cell: any, gridRow: any) => (
          <>{gridRow.Date.toDateString().slice(4)}</>
        ),
      },
      {
        dataField: "Points",
    ...
    return (
      <div className={styles.approvalsContainer}>
        {this.state.approveMessage && (
          <Label
            className={
              styles.approveMessage +
              " col-xl-5 col-lg-5 col-md-6 col-sm-8 col-xs-9"
            }
            aria-live="polite"
            role="alert"
          >
          ...
          search={{
            afterSearch: (newResult: IChampionPendingEvent[]) => {
              this.setState({
                ...
                isAllSelected:
                  newResult.length === this.state.selectedIds.length
                    ? true
                    : false,
              });
            },
            ...
            <div>
              {this.state.championPendingEvents.length > 0 && (
                <>
                  <div
                    className={
                      "col-xl-5 col-lg-5 col-md-6 col-sm-7 col-xs-9" +
                      " " +
                      styles.searchboxPadding
                    }
                  >
                    <SearchBox
                      placeholder={
                        LocaleStrings.PendingEventsSearchboxPlaceholder
                      }
                  ...
                  {this.state.selectedIds.length > 0 && (
                    <Label className={styles.selectedRowText}>
                      ...
                      {LocaleStrings.EventsSelectedLabel}
                    </Label>
                  )}
                </>
              )}
        ...
        {this.state.championPendingEvents.length > 0 && (
          <div className={styles.manageApprovalsBtnArea}>
            ...
            >
              ...
              <span className={styles.approveBtnLabel}>
                {LocaleStrings.ApproveButton}
              </span>
            </button>
            <button
              ...
              onClick={(e) =>
                this.updatePendingEvents(
                  stringConstants.rejectedStatus,
                  this.state.selectedIds
                )
              }
            ...
            >
              ...
              <span className={styles.rejectBtnLabel}>
                {LocaleStrings.RejectButton}
              </span>
            </button>
          </div>
        )}
