...
export default class ManageConfigSettings extends React.Component<
  IManageConfigSettingsProps,
  IManageConfigSettingsState
> {
  ...
  private async getConfigListSettings() {
    try {
      const configListData: IConfigList[] =
        await commonServiceManager.getAllItemsWithSpecificColumns(
  ...
  private async getMemberListColumnNames() {
    try {
      const columnsFilter =
        "InternalName eq '" +
        stringsConstants.RegionColumn +
        "' or InternalName eq '" +
        stringsConstants.CountryColumn +
        "' or InternalName eq '" +
        stringsConstants.GroupColumn +
        "'";
      const columnsDisplayNames: any[] =
        await commonServiceManager.getColumnsDisplayNames(
      ...
      console.error(
        "CMP_ManageConfigSettings_getMemberListColumnNames \n",
        error
      );
      this.setState({
        showError: true,
  ...
  private onToggleSetting(
    ...
    this.state.configListSettings.forEach((setting: IConfigList) => {
      if (setting.Title === settingName) {
        ...
        updatedSetting.Value = checked
          ? stringsConstants.EnabledStatus
          : stringsConstants.DisabledStatus;

        //Add/remove updated settings to/from updatedSettings state
        if (
          this.state.updatedSettings.find(
            (item: any) => item.id === updatedSetting.ID
          ) !== undefined
        ) {
    ...
    if (newTitle === "" || newTitle === undefined) {
      ...
      if (this.state.updatedSettings.length > 0) {
        ...
        commonServiceManager
          ...
          .catch((error) => {
            console.error(
              "CMP_ManageConfigSettings_saveConfigSettings \n",
              error
            );
            this.setState({
              showError: true,
          ...
          {this.state.configListSettings.length > 0 && (
            <>
              ...
              <TextField
                ...
                errorMessage={
                  this.state.appTitleError && LocaleStrings.AppTitleErrorLabel
                }
              />
              <Toggle
                checked={
                  this.state.configListSettings.find((setting: IConfigList) => {
                    return (
                      setting.Title === stringsConstants.ChampionEventApprovals
                    );
                ...
                label={
                  <div className={styles.toggleBtnLabel}>
                    ...
                    {this.iconWithTooltip(
                      ...
                      "configSettingsInfoIcon", //Class
                      "champion-event-approval-info-icon"
                    )}
                  </div>
                }
                inlineLabel
                ...
                onChange={(
                  ...
                  this.onToggleSetting(
                    ev,
                    checked,
                    stringsConstants.ChampionEventApprovals
                  )
                }
              ...
              <Label className={styles.listSettingLabel}>
                ...
                {this.iconWithTooltip(
                  ...
                  "configSettingsInfoIcon", //Class name
                  "member-list-field-info-icon"
                )}
              </Label>
              {this.state.memberListColumns.length > 0 && (
                <>
                  {this.state.memberListColumns.map((column) => {
                    const setting: IConfigList =
                      this.state.configListSettings.find(
                        (item: IConfigList) => {
                    ...
                    return (
                      <Toggle
                        checked={
                          setting.Value === stringsConstants.EnabledStatus
                        }
                        label={
                          <div
                            className={
                              styles.toggleBtnLabel + " " + styles.subLabel
                            }
                            aria-label={`${LocaleStrings.ShowHideToggle} ${column.Title} ${LocaleStrings.ToggleView}`}
                          >
                            {column.Title}
                          </div>
                        }
