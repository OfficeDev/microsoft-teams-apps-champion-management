...
> {
  ...
  public componentDidUpdate(
    prevProps: Readonly<ChampionEventsProps>,
    prevState: Readonly<ChampionEventsState>
  ): void {
    if (
      prevState.selectedUserActivities !== this.state.selectedUserActivities &&
      this.state.selectedUserActivities.length > 0
    ) {
      const paginationWrapper = document?.getElementsByClassName(
        "react-bootstrap-table-pagination"
      )[0];
      const divElements = paginationWrapper?.getElementsByTagName("div");
      divElements[0]?.setAttribute(
        "class",
        "col-md-8 col-xs-8 col-sm-8 col-lg-8"
      );
      divElements[1]?.setAttribute(
        "class",
        "react-bootstrap-table-pagination-list col-md-4 col-xs-4 col-sm-4 col-lg-4"
      );
    }
  }
  ...
  private async getChampionActivities(selectedChampion: string) {
    ...
    if (
      ...
      let selectedMemberEvents = this.props.filteredAllEvents.filter(
        (item) => item.MemberId === selectedChampion
      );
  ...
  customSortCaret = (order: any, column: any) => {
    ...
    if (!order) {
      ...
      if (column.dataField === stringsConstants.dateOfEventLabel) {
        document
          .getElementById(stringsConstants.dateOfEventId)
          .setAttribute("aria-sort", stringsConstants.sortAscAriaSort);
        document
          .getElementById(stringsConstants.eventTypeId)
          .setAttribute("aria-sort", "");
        document
          .getElementById(stringsConstants.eventPointsId)
          .setAttribute("aria-sort", "");
        document
          .getElementById(stringsConstants.dateOfEventId)
          .setAttribute("aria-description", stringsConstants.sortAscAriaSort);
        document
          .getElementById(stringsConstants.eventTypeId)
          .setAttribute("aria-description", "");
        document
          .getElementById(stringsConstants.eventPointsId)
          .setAttribute("aria-description", "");
      ...
      if (column.dataField === stringsConstants.dateOfEventLabel) {
        document
          .getElementById(stringsConstants.dateOfEventId)
          .setAttribute("aria-sort", stringsConstants.sortDescAriaSort);
        document
          .getElementById(stringsConstants.eventTypeId)
          .setAttribute("aria-sort", "");
        document
        ...
        document
          .getElementById(stringsConstants.dateOfEventId)
          .setAttribute("aria-description", stringsConstants.sortDescAriaSort);
        document
          .getElementById(stringsConstants.eventTypeId)
          .setAttribute("aria-description", "");
        document
  ...
  eventDateFormatter = (cell: any) => {
    ...
    return (
      <span aria-label={ariaLabel}>
        <span
          title={moment(new Date(cell)).format("MMMM Do, YYYY")}
          aria-hidden="true"
        >
  ...
  public render() {
    ...
    const isChampionReport =
      this.props.parentComponent === stringsConstants.ChampionReportLabel;
    ...
    const activitiesTableHeader: any = [
      {
        ...
        headerAttrs: {
          id: stringsConstants.dateOfEventId,
          role: "columnheader",
          scope: "col",
        },
      ...
      },
      {
        ...
        sortCaret: this.customSortCaret,
        headerAttrs: {
          id: stringsConstants.eventTypeId,
          role: "columnheader",
          scope: "col",
        },
        attrs: { role: "presentation" },
      },
      {
        ...
        sortCaret: this.customSortCaret,
        headerAttrs: {
          id: stringsConstants.eventPointsId,
          role: "columnheader",
          scope: "col",
        },
        attrs: { role: "presentation" },
      ...
      <div className="gtc-cards">
        <div className="showActivitiesPopupBody">
          <Row
            ...
            sm={1}
            xs={1}
            className="report-profile-grid-wrapper"
          >
            {this.props.parentComponent !== stringsConstants.SidebarLabel && (
              <Col
                xl={isChampionReport ? 3 : 4}
                lg={isChampionReport ? 3 : 4}
                md={isChampionReport ? 5 : 12}
                sm={12}
                xs={12}
                className={isChampionReport ? "report-profile-wrapper" : ""}
              >
                {this.state.selectedMemberDetails.length > 0 && (
                  <>
                    {isChampionReport && (
                      <div className="events-profile-heading">
                        {LocaleStrings.ChampionLabel}
                      </div>
                    )}
                    <div className="showActivitiesImage-IconArea">
                      ...
                      {this.props.parentComponent ===
                        stringsConstants.ChampionsCardsLabel && (
                        <div className="showActivities-rank-points-block">
                          <span
                            className="showActivities-rank"
                            title={`Rank ${this.state.selectedMemberDetails[0].Rank}`}
                          >
                            Rank{" "}
                            <span className="showActivities-rank-value">
                              # {this.state.selectedMemberDetails[0].Rank}
                            </span>
                          </span>
                          <span
                            className="showActivities-points"
                            title={`#${this.state.selectedMemberDetails[0].Points}`}
                          >
                            {this.state.selectedMemberDetails[0].Points}
                            <Icon
                              iconName="FavoriteStarFill"
                              className="showActivities-points-star"
                            />
                          </span>
                        </div>
                      )}
                      {this.props.loggedinUserEmail !==
                        ...
                        <div className="showActivities-icon-area">
                          <div
                            ...
                            onClick={() =>
                              this.openTask(
                                "https://teams.microsoft.com/l/meeting/new?subject=" +
                                  currentUserName +
                                  " / " +
                                  this.state.selectedMemberDetails[0]
                                    .FirstName +
                                  " " +
                                  this.state.selectedMemberDetails[0].LastName +
                                  " " +
                                  LocaleStrings.MeetupSubject +
                                  "&content=" +
                                  LocaleStrings.MeetupBody +
                                  "&attendees=" +
                                  this.state.selectedMemberDetails[0].Title
                              )
                            }
            ...
            <Col
              xl={isSidebar ? 12 : isChampionReport ? 7 : 8}
              lg={isSidebar ? 12 : isChampionReport ? 7 : 8}
              md={isChampionReport ? 7 : 12}
              sm={12}
              xs={12}
              className={isChampionReport ? "report-grid-wrapper" : ""}
            >
              {isChampionReport && (
                <div className="events-grid-heading">
                  {LocaleStrings.ActivitiesLabel}
                </div>
              )}
              <div className="showActivities-grid-area">
                {!isChampionReport && (
                  <div className="activities-grid-heading">
                    {LocaleStrings.ActivitiesLabel}
                  </div>
                )}
                <BootstrapTable
                  ...
                  wrapperClasses={
                    isDarkOrContrastTheme
                      ? "events-table-wrapper-class table-darkcontrast"
                      : "events-table-wrapper-class"
                  }
                  headerClasses={
                    isDarkOrContrastTheme ? "table-header-darkcontrast" : ""
                  }
                  noDataIndication={() => (
                    <div className="activities-noRecordsFound">
                      {LocaleStrings.NoActivitiesinGridLabel}
                    </div>
                  )}
                />
              </div>
