...
export default function ChampionLeaderBoard(props: ChampionLeaderBoardProps) {
  ...
  const [eventsSubmissionMessage, setEventsSubmissionMessage] =
    React.useState("");
  const isDarkOrContrastTheme =
    props.currentThemeName === constants.themeDarkMode ||
    props.currentThemeName === constants.themeContrastMode;
      ...
      {isLoaded && (
        <div className="Championleaderboard d-flex">
          ...
          <div className="content-tab">
            <div
              className={`ClbPath${
                isDarkOrContrastTheme ? " ClbPathDarkContrast" : ""
              }`}
            >
              <img
              ...
              <span
                ...
                tabIndex={0}
                onKeyDown={(evt: any) => {
                  if (evt.key === constants.stringEnter) props.onClickCancel();
                }}
                aria-label={props.appTitle}
              >
                <span title={props.appTitle}>{props.appTitle}</span>
              </span>
              <span className="ClbBorder"></span>
              <span className="ClbLabel">
                {LocaleStrings.ChampionLeaderBoardLabel}
              </span>
            </div>
            {eventsSubmissionMessage !== "" && (
              <Label
                className={`events-success-message${
                  isDarkOrContrastTheme
                    ? " events-success-messageDarkContrast"
                    : ""
                }`}
                aria-live="polite"
                role="status"
